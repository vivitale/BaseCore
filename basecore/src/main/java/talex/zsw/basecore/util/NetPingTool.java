...
public class NetPingTool
{
	 ...
	 * 初始化网络诊断服务
	 */
	public NetPingTool(Context context, String domain, IOnNetPingListener theListener)
	{
		this(context, domain, 80, theListener);
	}

	public NetPingTool(Context context, String domain, int port, IOnNetPingListener theListener)
	{
		mPort = port;
		this.mDomain = domain;
	...
	private boolean execIP(InetAddress inetAddress, String ip)
	{
		...
		{
			socketAddress = new InetSocketAddress(inetAddress, mPort);
			int flag = 0;
			...
			if(!isConnected && mIOnNetPingListener != null)
			{
				mIOnNetPingListener.onError();
			}
		}
