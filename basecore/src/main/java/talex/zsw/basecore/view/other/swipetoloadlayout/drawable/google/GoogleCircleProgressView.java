...
public class GoogleCircleProgressView extends ImageView
{
	...
	{
		final TypedArray a
			= context.obtainStyledAttributes(attrs, R.styleable.GoogleCircleProgressView, defStyleAttr, 0);
		final float density = getContext().getResources().getDisplayMetrics().density;

		mBackGroundColor
			= a.getColor(R.styleable.GoogleCircleProgressView_gcp_background_color, DEFAULT_CIRCLE_BG_LIGHT);

		mProgressColor = a.getColor(R.styleable.GoogleCircleProgressView_gcp_progress_color, DEFAULT_CIRCLE_BG_LIGHT);
		mColors = new int[]{mProgressColor};

		mInnerRadius = a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_inner_radius, -1);

		mProgressStokeWidth
			= a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_progress_stoke_width, (int) (
			STROKE_WIDTH_LARGE*density));
		mArrowWidth = a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_arrow_width, -1);
		mArrowHeight = a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_arrow_height, -1);
		mTextSize = a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_progress_text_size, (int) (
			DEFAULT_TEXT_SIZE*density));
		mTextColor = a.getColor(R.styleable.GoogleCircleProgressView_gcp_progress_text_color, Color.BLACK);
	...
	@Override protected void onLayout(boolean changed, int left, int top, int right, int bottom)
	{
		...
		mProgressDrawable.setSizeParameters(mDiameter, mDiameter,
		                                    mInnerRadius <= 0 ? (mDiameter-mProgressStokeWidth*2)/4 :
			                                    mInnerRadius, mProgressStokeWidth,
	...
	@Override final public void setImageResource(int resId)
	{
	 ...
	 */
	@SuppressLint("ResourceType") public void setBackgroundColor(@ColorRes int colorRes)
	{
	...
	private class OvalShadow extends OvalShape
	{
		...
		public OvalShadow(int shadowRadius, int circleDiameter)
		{
			...
			mRadialGradient = new RadialGradient(
				mCircleDiameter/2, mCircleDiameter/2, mShadowRadius, new int[]{FILL_SHADOW_COLOR,
