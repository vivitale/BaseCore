package talex.zsw.basecore.view.other.swipetoloadlayout;
...
public class SwipeToLoadLayout extends ViewGroup
{
	 ...
	 */
	private int mReleaseToRefreshingScrollingDuration =
	 ...
	 */
	private int mReleaseToLoadingMoreScrollingDuration =
			...
			for (int i = 0; i < N; i++)
			{
				...
				else if (attr == R.styleable.SwipeToLoadLayout_refresh_complete_delay_duration)
				{
					setRefreshCompleteDelayDuration(
						a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));
				}
				else if (attr ==
				...
				{
					setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr,
						DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));
				}
				else if (attr ==
	...
	{
		if (mTargetHead != null && mTargetHead != view)
		{
			removeView(mTargetHead);
		}
		if (mTargetHead != view)
		{
			this.mTargetHead = view;
			addView(view);
		}
	}
		...
		else
		{
			...
			if (STATUS.isRefreshing(mStatus))
			{
				...
				postDelayed(new Runnable()
				{
					...
					{
						mAutoScroller.autoScroll(-mHeaderOffset,
							mRefreshCompleteToDefaultScrollingDuration);
					}
		...
		else
		{
			if (STATUS.isLoadingMore(mStatus))
			{
				...
				postDelayed(new Runnable()
				{
					@Override public void run()
					{
						mAutoScroller.autoScroll(-mFooterOffset,
			...
			{
				mAutoScroller
					.autoScroll(-mFooterOffset, mLoadMoreCompleteToDefaultScrollingDuration);
			}
		}
	}
	...
	{
		return new SwipeToLoadLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
	...
	@Override protected void onFinishInflate()
	{
		...
		else if (0 < childNum && childNum < 5)
		{
			...
			mTargetHead = findViewById(R.id.swipe_head);
	...
	@Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		if (mTargetHead != null)
		{
			final View targetView = mTargetHead;
	...
	private void layoutChildren()
	{
		...
		if (mTargetHead != null)
		{
			final View head = mTargetHead;
			MarginLayoutParams lp = (MarginLayoutParams) head.getLayoutParams();
			...
			final int targetRight = targetLeft + head.getMeasuredWidth();
			final int targetBottom = targetTop + head.getMeasuredHeight();
			head.layout(targetLeft, targetTop, targetRight, targetBottom);
		}
		...
		if (mTargetView != null)
		{
			...
			if (mTargetHead != null)
			{
				HeadHeight = mTargetHead.getMeasuredHeight();
			}
			...
			switch (mStyle)
			{
				case STYLE.CLASSIC:
					// classic
					targetTop = paddingTop + lp.topMargin + mTargetOffset + HeadHeight;
					break;
				case STYLE.ABOVE:
					// above
					targetTop = paddingTop + lp.topMargin + HeadHeight;
					break;
				case STYLE.BLEW:
					// classic
					targetTop = paddingTop + lp.topMargin + mTargetOffset + HeadHeight;
			...
			final int targetBottom = targetTop + targetView.getMeasuredHeight() - HeadHeight;
		...
		if (mFooterView != null)
		{
			...
			switch (mStyle)
			{
				...
				case STYLE.SCALE:
					// scale
					footerBottom = height - paddingBottom - lp.bottomMargin + mFooterHeight / 2 +
						mFooterOffset / 2;
					break;
				default:
					// classic
	...
	private boolean onCheckCanRefresh()
	{
		...
		return mRefreshEnabled && !canChildScrollUp() && mHasHeaderView &&
			mRefreshTriggerOffset > 0;
	}
	...
	private boolean onCheckCanLoadMore()
	{
		...
		return mLoadMoreEnabled && !canChildScrollDown() && mHasFooterView &&
	...
	private void fingerScroll(final float yDiff)
	{
		...
		else if (mLoadMoreFinalDragOffset >= mLoadMoreTriggerOffset &&
			-tmpTargetOffset > mLoadMoreFinalDragOffset)
		{
	...
	private void updateScroll(final float yScrolled)
	{
		...
		else if (mTargetOffset == 0)
		{
			...
			else
			{
				...
				else if (STATUS.isLoadMoreComplete(mStatus) ||
	...
	private final static class STATUS
	{
		...
		@SuppressWarnings({"unused"}) private static boolean isRefreshReturning(final int status)
		{
			return status == STATUS.STATUS_REFRESH_RETURNING;
		}

		@SuppressWarnings({"unused"}) private static boolean isLoadMoreReturning(final int status)
		{
