...
public class AppTool
{
	...
	{
		if(RegTool.isNullString(packageName))
		{
			return false;
		}
		try
	...
	{
		return !RegTool.isNullString(packageName) && packageName.equals(ProcessTool.getForegroundProcessName(context));
	}
	//----------------------------------------------------------------------------------------------------------------
	...
	{
		boolean isSuccess = FileTool.cleanInternalCache(context);
		isSuccess &= FileTool.cleanInternalDbs(context);
		isSuccess &= FileTool.cleanInternalSP(context);
		isSuccess &= FileTool.cleanInternalFiles(context);
		isSuccess &= FileTool.cleanExternalCache(context);
		for(File dir : dirs)
		{
			isSuccess &= FileTool.cleanCustomCache(dir);
		}
		return isSuccess;
	}
	...
	{
		PackageManager packageManager = Tool.getContext().getPackageManager();
		Intent intent = packageManager.getLaunchIntentForPackage(Tool.getContext().getPackageName());
		if(intent == null)
		{
			return;
		}
