...
public class PopListView extends PopupWindow
{
	...
	public PopListView(Context context, int width, int height)
	{
		...
		// 获得屏幕的宽度和高度
		mScreenWidth = DeviceTool.getScreenWidth(mContext);
		mScreenHeight = DeviceTool.getScreenHeight(mContext);
		...
		// 设置弹窗的布局界面
		setContentView(LayoutInflater.from(mContext).inflate(R.layout.pop_listview, null));

		initUI();
	}

	public PopListView(Context context, int width, int height, int layout)
	{
		this.mContext = context;
	...
	public void show(View view)
	{
		...
		showAtLocation(view, popupGravity,
		               mScreenWidth-LIST_PADDING-(getWidth()/2), mRect.bottom+DimenTool.dp2px(7.5f));
	}
