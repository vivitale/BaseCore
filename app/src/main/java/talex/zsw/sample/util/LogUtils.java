...
public final class LogUtils
{
		...
		else
		{
			...
			if(sLogHeadSwitch)
			{
				...
				final String head = new Formatter()
					.format("%s, %s(%s:%d)", tName, targetElement.getMethodName(), fileName, targetElement.getLineNumber())
					.toString();
				...
				else
				{
					...
					for(int i = 1, len = consoleHead.length; i < len; ++i)
					{
						...
						consoleHead[i] = new Formatter()
							.format("%s%s(%s:%d)", space, targetElement.getMethodName(), targetElement.getFileName(), targetElement
								.getLineNumber())
							.toString();
					}
	...
	private static boolean delLog(String path)
	{
		boolean flag = false;
		...
		if(files!=null)
		{
			for(int i = 0; i < files.length; i++)
			{
				...
				int len = sFilePrefix.length();
				if(name.startsWith(sFilePrefix) && name.length() == (len+13))
				{
					String fileDateStr = name.substring(len+1, len+3)+name.substring(len+4, len+6);
					int fileDate = DataTool.string2Int(fileDateStr);
					Date now = new Date(System.currentTimeMillis());
					Format FORMAT = new SimpleDateFormat("MMdd", Locale.getDefault());
					String date = FORMAT.format(now);
					int nowDate = DataTool.string2Int(date);
					if(nowDate-fileDate > 7)
					{
						file("filename = "+exeFile.getName()+" 删除");
						exeFile.delete();
					}
				}
			}
	...
	public static List<File> listLogs()
	{
	...
	public static class Config
	{
		private Config()
		{
			...
			{
				sDefaultDir = Environment.getExternalStorageDirectory()+FILE_SEP+"log"+FILE_SEP;
			}
			else if(Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) &&
				Tool.getContext().getExternalCacheDir() != null)
			{
				sDefaultDir = Tool.getContext().getExternalCacheDir()+FILE_SEP+"log"+FILE_SEP;
			}
			else
			{
				sDefaultDir = Tool.getContext().getCacheDir()+FILE_SEP+"log"+FILE_SEP;
			}
		}
		...
		{
			return "switch: "+sLogSwitch+LINE_SEP+"console: "+sLog2ConsoleSwitch+LINE_SEP+"tag: "+
				(sTagIsSpace ? "null" : sGlobalTag)+LINE_SEP+"head: "+sLogHeadSwitch+LINE_SEP+"file: "+sLog2FileSwitch+
				LINE_SEP+"dir: "+(sDir == null ? sDefaultDir : sDir)+LINE_SEP+"filePrefix"+sFilePrefix+LINE_SEP+
				"border: "+sLogBorderSwitch+LINE_SEP+"consoleFilter: "+T[sConsoleFilter-V]+LINE_SEP+"fileFilter: "+
				T[sFileFilter-V]+LINE_SEP+"stackDeep: "+sStackDeep;
		}
	}
