package talex.zsw.basecore.view.dialog.sweetalertdialog;
...
public class ProgressWheel extends View
{
	...
	private void parseAttributes(TypedArray a)
	{
		...
		circleRadius = (int) a.getDimension(
				R.styleable.ProgressWheel_SAD_matProg_circleRadius, circleRadius);

		fillRadius = a.getBoolean(R.styleable.ProgressWheel_SAD_matProg_fillRadius,
				false);

		barWidth = (int) a.getDimension(
				R.styleable.ProgressWheel_SAD_matProg_barWidth, barWidth);

		rimWidth = (int) a.getDimension(
				R.styleable.ProgressWheel_SAD_matProg_rimWidth, rimWidth);

		float baseSpinSpeed = a
				.getFloat(R.styleable.ProgressWheel_SAD_matProg_spinSpeed,
						spinSpeed / 360.0f);
		spinSpeed = baseSpinSpeed * 360;

		barSpinCycleTime = a.getInt(
				R.styleable.ProgressWheel_SAD_matProg_barSpinCycleTime,
				(int) barSpinCycleTime);

		barColor = a.getColor(R.styleable.ProgressWheel_SAD_matProg_barColor,
				barColor);

		rimColor = a.getColor(R.styleable.ProgressWheel_SAD_matProg_rimColor,
				rimColor);

		linearProgress = a.getBoolean(
				R.styleable.ProgressWheel_SAD_matProg_linearProgress, false);

		if (a.getBoolean(
				R.styleable.ProgressWheel_SAD_matProg_progressIndeterminate, false))
		{
			spin();
		}
	 ...
	 *         (1.0 equals on full turn in one second), this value also is
	 *         applied for
	...
	public interface ProgressCallback
	{
		 ...
		 * the wheel completes an animation cycle, with, the progress value is
		 * -1.0f
		 *
		 * @param progress
