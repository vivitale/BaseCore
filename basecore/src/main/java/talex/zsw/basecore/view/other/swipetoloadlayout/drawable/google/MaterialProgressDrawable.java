 ...
 * 修改备注：
 */
@SuppressWarnings("ALL") public class MaterialProgressDrawable extends Drawable
	implements Animatable
{
	...
	public void updateSizes(@ProgressDrawableSize int size)
	{
		...
		if(size == LARGE)
		{
			setSizeParameters( CIRCLE_DIAMETER_LARGE * screenDensity,
				CIRCLE_DIAMETER_LARGE * screenDensity, CENTER_RADIUS_LARGE * screenDensity,
				STROKE_WIDTH_LARGE * screenDensity, ARROW_WIDTH_LARGE * screenDensity,
				ARROW_HEIGHT_LARGE * screenDensity );
		}
		else
		{
			setSizeParameters( CIRCLE_DIAMETER * screenDensity, CIRCLE_DIAMETER * screenDensity,
				CENTER_RADIUS * screenDensity, STROKE_WIDTH * screenDensity,
	...
	@Retention(RetentionPolicy.CLASS) @IntDef({LARGE, DEFAULT})
	...
	private static class Ring
	{
		...
		private void drawTriangle(Canvas c, float startAngle, float sweepAngle, Rect bounds)
		{
			if(mShowArrow)
			{
				...
				c.rotate( startAngle + (sweepAngle < 0 ? 0 : sweepAngle) - ARROW_OFFSET_ANGLE,
